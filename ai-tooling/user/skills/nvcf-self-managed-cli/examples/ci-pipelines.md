...
deploy_nvcf:
  ...
  script:
    - nvcf-cli self-hosted check --pre --json 2>&1 >/dev/null | grep '^{' | jq -e '.event != "phase_failed"' || exit 2
    - nvcf-cli self-hosted up --cluster-name=$CLUSTER_NAME --token=$NVCF_ADMIN_JWT --non-interactive --json
    - nvcf-cli self-hosted status --json 2>&1 >/dev/null | grep '^{' | jq -e '.verdict == "healthy"'
```
