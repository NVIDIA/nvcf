...
- `nvcf-cli task delete` — permanently removes the task record. Stop and state the task ID and current status, then wait for an explicit last-look confirmation before running this command. Do not treat the user's original delete request as confirmation — always get a separate explicit "yes, delete it" in the same turn before proceeding.
