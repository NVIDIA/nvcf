...
func newPurgeTestTaskRequest(instanceID, taskID string) (*nvcav2beta1.ICMSRequest, nvcav2beta1.InstanceStatus) {
	req := &nvcav2beta1.ICMSRequest{
		Spec: nvcav2beta1.ICMSRequestSpec{
			RequestID: "req-1",
...
// Regression test for two task requests sharing a budget: task requests
// populate ICMSRequestSpec.TaskDetails, not FunctionDetails, so deriving
// the budget key from FunctionDetails unconditionally gave every task an
// identical, empty identity ("nvca-recreation-budget--"), letting an
// unrelated task's failures exhaust the budget and block every other
// task's purges for 15 minutes.
func Test_GetICMSRequestUpdatesForMiniServiceRequest_TaskPurgesUseIndependentBudgets(t *testing.T) {
	...
	// Exhaust task-1's budget directly.
	for i := 0; i < recreationBudgetMaxPurges; i++ {
		allowed, _, terr := c.tryReserveRecreationSlot(context.Background(), recreationBudgetKindTask, "task-1", "")
		require.NoError(t, terr)
		require.True(t, allowed)
	}
	allowed, _, err := c.tryReserveRecreationSlot(context.Background(), recreationBudgetKindTask, "task-1", "")
	require.NoError(t, err)
	require.False(t, allowed, "task-1's budget should now be exhausted")

	// task-2 must still get its own, independent budget: its purge must
	// not be blocked by task-1 having exhausted a shared key.
	instanceID := "sr-task-2"
	createPurgeTestMiniService(t, clients.HelmV2, instanceID)
	req, st := newPurgeTestTaskRequest(instanceID, "task-2")
	...
	assert.True(t, apierrors.IsNotFound(err),
		"task-2 must be purged despite task-1's budget being exhausted -- proving independent per-task budgets, not a shared empty-key budget")
}
...
func Test_GetICMSRequestUpdatesForMiniServiceRequest_SkipsPurgeWhenBudgetExhausted(t *testing.T) {
	...
	cmName := recreationBudgetConfigMapName(recreationBudgetKindFunction, testFunctionID, testFunctionVersionID)
...
func Test_GetICMSRequestUpdatesForMiniServiceRequest_SkipsPurgeWhenBudgetCheckErrors(t *testing.T) {
	...
	fakeK8s.PrependReactor("get", "configmaps", func(action k8stesting.Action) (bool, runtime.Object, error) {
		...
		if !ok || getAction.GetName() != recreationBudgetConfigMapName(recreationBudgetKindFunction, testFunctionID, testFunctionVersionID) {
