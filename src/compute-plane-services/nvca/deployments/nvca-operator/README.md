...
| `controlPlane.id`                         | Optional lowercase DNS label identifying an isolated control plane. Empty preserves legacy names; a value such as `plane-a` requires release and namespace `plane-a-nvca-operator`.                  | `""`                       |
...
| `agent.secretMirrorNamespace`        | Namespace to source mirrored workload secrets from; empty derives this control plane's operator namespace. A custom namespace is an intentional shared/alternate source and must be access-controlled. | `""`                   |
