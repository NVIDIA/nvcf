...
func validateReadyPersistedModelCacheStorageRequest(
	...
	if selection.BindingName == "" || selection.BindingUID == "" {
		return fmt.Errorf("durable Helm model cache selection has no committed binding reference")
	}
	...
	if got := st.Labels[nvcastorage.ModelCacheBindingUIDLabelKey]; got != string(selection.BindingUID) {
		return fmt.Errorf(
			"durable model cache StorageRequest %s/%s has binding UID %q, want %q",
			st.Namespace, st.Name, got, selection.BindingUID)
	}
	return nil
}
