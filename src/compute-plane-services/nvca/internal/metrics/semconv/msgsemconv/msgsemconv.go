...
// set for messaging operations (for example SQS and NATS). It is scaffolded for
// the follow-on queue-client instrumentation; NVCA has no consumer wired yet.
package msgsemconv
...
const (
	...
	OperationKey       = attribute.Key("messaging.operation")
	DestinationNameKey = attribute.Key("messaging.destination.name")
	BatchMessageCount  = attribute.Key("messaging.batch.message_count")
)
...
// the messaging call; batchCount is included when greater than zero; errType is
// included only on failure.
func ClientAttrs(peerService, system, operation, destination string, batchCount int, errType string) []attribute.KeyValue {
	...
	if batchCount > 0 {
		attrs = append(attrs, BatchMessageCount.Int(batchCount))
	}
	if errType != "" {
