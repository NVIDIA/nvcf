...
#
# Builds and runs the --gpu-map parsing tests.
#
...
if command -v gcc >/dev/null 2>&1 && [ -f /usr/local/cuda/include/cuda.h ] && \
   grep -q "gpuPairs" /usr/local/cuda/include/cuda.h 2>/dev/null; then
    ...
    gcc -O2 -Wall -o /tmp/nvsnap-gpu-map-test "$HERE/gpu_map_test.c" \
        -I/usr/local/cuda/include
    exec /tmp/nvsnap-gpu-map-test
fi

echo "Building in $IMAGE"
exec docker run --rm -v "$HERE/..:/src:ro" "$IMAGE" bash -c '
    gcc -O2 -Wall -o /tmp/t /src/tests/gpu_map_test.c -I/usr/local/cuda/include && exec /tmp/t
'
