...
// does not emit target_info.
//
// The series is generated by the exporter from resource attributes only, so it
// never carries the function, instance or account labels that metricstransform
// adds as datapoint labels. After the resource processor deletes
// service.instance.id it reduces to job, server_address and url_scheme, which
// are identical for every collector federating the same upstream on a node.
// Multiple collectors then write a byte-identical series into one tenant, and a
// Prometheus-compatible receiver rejects the conflicting write.
	...
	for _, provider := range []string{string(ProviderThanos), string(ProviderPrometheus)} {
		t.Run(provider, func(t *testing.T) {
			...
			for id, exporter := range cfg.Exporters {
				if !strings.HasPrefix(id, "prometheus_remote_write/") {
					continue
				}
				checked++
