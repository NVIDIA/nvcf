...
func ExitReason(err error) {
	...
	errorLog := err.Error()
	if workerErr, ok := err.(*types.WorkerError); ok {
		output, err := json.Marshal(workerErr)
		if err != nil {
			zap.L().Error("failed to marshal worker error", zap.Error(err))
			return
		}
		errorLog = string(output)
	}

	if err := writeTerminationLog(string(errorLog)); err != nil && err != ErrNotInKubernetesEnv {
		zap.L().Error("failed to write termination log", zap.Error(err))
	}
}
