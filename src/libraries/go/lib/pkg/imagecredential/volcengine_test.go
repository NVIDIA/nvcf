...
func TestVolcEngineRunErrors(t *testing.T) {
	ctx := t.Context()
	refURL, err := parseRef("project-myreg1-cn-shanghai.cr.volces.com/repo:tag")
	require.NoError(t, err)

	_, _, err = volcengineHelper{}.Run(ctx, refURL, AuthHelperCredentials{LoadFromEnv: true})
	assert.EqualError(t, err, "loading credentials from environment is not supported for VolcEngine")

	invalidURL, err := parseRef("registry.example.com/repo:tag")
	require.NoError(t, err)
	_, _, err = volcengineHelper{}.Run(ctx, invalidURL, AuthHelperCredentials{Username: "key", Password: "secret"})
	assert.EqualError(t, err, "no VolcEngine registry match")

	_, _, err = volcengineHelper{
		newClient: func(_ context.Context, _, _ string, _ AuthHelperCredentials) (volcengineCRAPI, error) {
			return nil, errors.New("create volcengine client")
		},
	}.Run(ctx, refURL, AuthHelperCredentials{Username: "key", Password: "secret"})
	assert.EqualError(t, err, "create volcengine client")

	_, _, err = volcengineHelper{
		newClient: func(_ context.Context, _, _ string, _ AuthHelperCredentials) (volcengineCRAPI, error) {
			return fakeVolcEngineClient{
				auths: map[string]volcenginecr.GetAuthorizationTokenOutput{
					"project-myreg1": {Token: volcengine.String("token")},
				},
			}, nil
		},
	}.Run(ctx, refURL, AuthHelperCredentials{Username: "key", Password: "secret"})
	assert.EqualError(t, err, "empty username or token")
}
