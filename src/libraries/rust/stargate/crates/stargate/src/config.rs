...
impl StargateConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if let Some(kubernetes) = &self.stargate_identity.kubernetes {
            ...
            !self
                .process_lifecycle
                .readiness_stabilization_sample_interval
                .is_zero(),
            "process_lifecycle.readiness_stabilization_sample_interval_ms must be greater than 0"
        );
        ensure!(
            self.process_lifecycle.readiness_stabilization_window > 0,
            "process_lifecycle.readiness_stabilization_window must be greater than 0"
        );
...
pub struct ProcessLifecycleConfig {
    ...
    #[serde_as(as = "DurationMilliSeconds<u64>")]
    #[serde(rename = "readiness_stabilization_sample_interval_ms")]
    pub readiness_stabilization_sample_interval: Duration,
    pub readiness_stabilization_window: u32,
    #[serde_as(as = "DurationMilliSeconds<u64>")]
...
impl Default for ProcessLifecycleConfig {
    fn default() -> Self {
        let warmup = crate::runtime::WarmupConfig::default();
        Self {
            readiness_warmup: warmup.warmup_duration,
            readiness_stabilization_sample_interval: warmup.sample_interval,
            readiness_stabilization_window: warmup.stabilization_window,
...
"#;
    ...
    fn minimal_config_uses_runtime_defaults_and_self_only_membership() {
        ...
        assert_eq!(config.process_lifecycle.readiness_warmup, Duration::ZERO);
        assert_eq!(
            config
                .process_lifecycle
                .readiness_stabilization_sample_interval,
        ...
        assert_eq!(config.process_lifecycle.readiness_stabilization_window, 5);
...
"#,
    ...
    #[test]
    fn readiness_stabilization_values_must_be_nonzero() {
        for (field, expected) in [
            (
                "readiness_stabilization_sample_interval_ms = 0",
                "readiness_stabilization_sample_interval_ms must be greater than 0",
            ),
            (
                "readiness_stabilization_window = 0",
                "readiness_stabilization_window must be greater than 0",
            ),
        ] {
            let source = format!("{MINIMAL}\n[process_lifecycle]\n{field}\n");
            let error = StargateConfig::from_toml_str(&source)
                .expect_err("zero stabilization values should be rejected");
