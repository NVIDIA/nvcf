...
func TestClientIntrospectCachesActiveValidSubjectOnly(t *testing.T) {
	calls := 0
	...
	// Second call for the same token must hit the cache, not the server.
...
func TestClientIntrospectDoesNotCacheMissingClusterID(t *testing.T) {
	calls := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls++
		w.Header().Set("Content-Type", "application/json")
