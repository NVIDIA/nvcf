...
class FunctionDeploymentsTaskTest {
    ...
    void beforeEach() {
        functionDeploymentsTask = new FunctionDeploymentsTask(
                ...
                new FunctionDeploymentsRegionProvider(
                        "us-east-1", List.of("us-east-1")),
                tracer,
    ...
    @Test
    void shouldAssignEachDeploymentToExactlyOneRegion() {
        var regions = List.of("us-east-1", "us-west-2", "eu-west-1");
        var regionProviders = regions.stream()
                .map(region -> new FunctionDeploymentsRegionProvider(region, regions))
                .toList();

        for (var index = 0; index < 12; index++) {
            var deployment = FunctionDeploymentEntity.builder()
                    .key(FunctionDeploymentKey.builder()
                            .functionVersionId(UUID.nameUUIDFromBytes(
                                    ("version-" + index).getBytes(StandardCharsets.UTF_8)))
                            .build())
                    .deploymentId(TEST_DEPLOYMENT_ID)
            ...
            assertThat(regionProviders
                               .stream()
                               .filter(provider -> provider.owns(deployment)).count())
                    .isOne();
        }
    }
}
