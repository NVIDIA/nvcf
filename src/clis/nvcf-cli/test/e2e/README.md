...
Spec §8.4 of the [combined SRD/SDD](https://github.com/NVIDIA/mcamp/docs/-/blob/main/nvcf/multi-cluster-registration/one-click-deploy-self-hosted-nvcf-srd-sdd.md) — the T1–T7 matrix that gates each release.

## Prerequisites

- `mcamp-dev-vm` (or any host with k3d + helmfile + helm + a working kubectl context).
...
- T3 timing (90 s SIGINT delay) targets the post-control-plane register window
  on `mcamp-dev-vm`. On slower hosts the interrupt may land mid-control-plane;
...
context routing in `up` / `status`. Run on `mcamp-dev-vm`; not in CI.
