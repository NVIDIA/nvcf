...
- `llmRequestRouter.transport.grpcPylonDialAddr` and `llmRequestRouter.transport.reverseTunnelPylonDialAddr` for the shared `host:port` addresses pylon dials when the router is reached from outside the cluster. Leave both empty for in-cluster installs, where pylon dials the advertised per-pod address directly. Setting them does not expose the router; the TCP and UDP paths must be routed separately.
- `llmRequestRouter.kubernetes.advertisedHostnameTemplate` for the per-pod identity the router advertises as the gRPC authority and QUIC SNI. Supports `{pod_name}` and `{namespace}`. Override it alongside the dial addresses so external pylons receive a resolvable identity. Above one replica, whatever terminates the dial addresses must route on this identity or reverse tunnels will land on a router pod that does not own the tunnel target.
