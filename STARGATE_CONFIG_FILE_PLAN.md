...
| process_lifecycle | Process readiness warm-up, stabilization, and shutdown drain timeout |
...
- readiness_warmup_ms is the maximum time before the readiness endpoint
  accepts request traffic. Zero disables the warm-up. A stable non-zero
  backend count can end the warm-up earlier.
- readiness_stabilization_sample_interval_ms controls how often Stargate
  samples the active backend count during warm-up.
- readiness_stabilization_window is the number of consecutive stable,
  non-zero samples required to end warm-up early.
...
| --readiness-stabilization-sample-interval-ms | process_lifecycle.readiness_stabilization_sample_interval_ms |
| --readiness-stabilization-window | process_lifecycle.readiness_stabilization_window |
