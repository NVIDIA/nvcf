...
The CLI stores two bearer credential types:

- `NVCF_TOKEN`: Generated by `nvcf-cli init`. This is the default CLI
  credential for management operations and self-hosted cluster management.
- `NVCF_API_KEY`: Generated by `nvcf-cli api-key generate`. This is the
  default CLI credential for function invocation, function discovery, and queue
  status.
...
are configured. Self-hosted SIS cluster management uses `NVCF_TOKEN`.
