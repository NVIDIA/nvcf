...
1. Stand up the new datacenter joined to the existing cluster. Deploy the 0.6.1
   ...
   bootstrap. Do not run the schema-initialization or migration steps against
   the new datacenter: the schema already exists in the cluster and the new
   nodes receive it through gossip. Re-running them can fail or race the
   existing schema.
