...
| `stargate_backend_loss_cancellations_total` | Counter | `llm-request-router:9090/metrics` | `routing_key`, `model`, `outcome` | In-flight requests cancelled because their backend registration was lost. `outcome` is `re_dispatched` (retried on another backend), `no_alternative` (retry granted, no eligible backend left), `terminated` (response already started), or `failed` (retry refused by budget, cap, or replay readiness). |
