...
| `x-request-slo-ms` | Not generated by the stock gateway. A trusted upstream can supply the queue-SLO duration on native proxy paths. This does not shorten the configured affinity wait. |
| `x-max-wait-ms` | Not generated by the stock gateway. A trusted upstream can supply a routing wait limit on native proxy paths, including a limit shorter than the affinity wait. |
...
A configured WaitAndWiden `cache_affinity_wait_ms` keeps public candidates
closed until that duration has elapsed from request arrival. It works without
either deadline header. The router continues checking affine capacity during
the wait. An explicit shorter `x-max-wait-ms` can end routing with HTTP `503`
before public candidates become eligible. Configure the affinity wait within
the latency budget for the workload.
