...
This page covers the compute plane stack: the NVCA operator, GPU cluster
registration, and cleanup of NVCA components. For control-plane issues such as
account bootstrap, Cassandra, OpenBao, or gateway routing, see
[Self-Managed Troubleshooting](/nvcf/self-managed/troubleshooting). For the
registration procedure itself, see
[Register a GPU Cluster](./cluster-management/self-managed.md).
...
If NVCF services are also broken, follow the "Recovering from Services Failures" steps in [Helmfile Installation](/nvcf/self-managed/helmfile-installation).

## NVCA Force Cleanup Script

If `helmfile destroy` hangs on NVCA cleanup (typically when functions are still deployed in `nvcf-backend`), use the force cleanup script in a new terminal. See [the appendix](#appendix-nvca-force-cleanup-script) for the full script and usage instructions.
