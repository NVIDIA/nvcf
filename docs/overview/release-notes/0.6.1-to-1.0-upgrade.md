...
The validated path has two data stops and no intermediate application-runtime
stop:

1. Upgrade Cassandra and run schema migrations 0.10.3 to 0.17.6 while the
   control-plane services are quiesced.
2. Upgrade OpenBao, refresh its JWT plugin catalog, rotate its `OnDelete`
   StatefulSet, and run OpenBao migrations 0.16.2 to 0.19.5.
3. Sync the target control-plane and observability releases directly.
4. Re-register each existing compute cluster without deleting its identity,
   then sync compute-plane 0.4.4 and NVCA 3.10.0.

The application runtimes can move directly between these versions:

| Service | Source runtime | Target runtime | Intermediate stop |
| --- | --- | --- | --- |
| NVCF API | 1.9.0-hotfix.1 | 1.18.0 | None |
| NVCT API | 1.5.9-hotfix.1 | 1.66.0 | None |
| SIS / ICMS | 1.563.1-hotfix.1 | 0.7.2 | None |
| ESS API | 0.57.26 | 0.5.0 | None |
| NVCA | 3.0.3 | 3.10.0 | None |
...
No temporary scheduled-task setting is required for this path. NVCA 3.0.3
writes both the `gpus_v4` and `gpus_v5` inventory fields when it registers a
cluster. The legacy SIS backfill task does not run in the 0.6.1 self-hosted
profile. Verify `gpus_v5` before the upgrade. If it is empty for an active
cluster, stop and contact NVIDIA support instead of enabling an undocumented
task setting.
...
## Stop 1: migrate Cassandra

Run only the Cassandra release from the target control-plane bundle:
...
The command must finish successfully. Confirm that the Cassandra migration Job
