...
before the maintenance window. Use the inventory JSON attached to each 1.0.0
...
one pod. Kubernetes calculates those values as 0 and 1 from the default 25
percent settings when a Deployment has two replicas. Do not claim a
zero-downtime Cassandra server upgrade with fewer than three healthy members.
...
- Re-registered the existing split compute cluster without changing its NCA,
  cluster, or cluster-group identity, then upgraded it to compute-plane 1.0.0
