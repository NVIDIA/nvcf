...
| `And I prepare self-managed secrets file {string} from template {string} using the current NGC registry credential` | The destination and template are explicit repo-relative paths with `${VAR}` interpolation. Copies the template permissions and replaces its registry credential placeholder with base64 of the current `$oauthtoken:<NGC_API_KEY>` credential. The destination is ledger-backed, and secret material never enters Gherkin, command logs, or failure messages. |
