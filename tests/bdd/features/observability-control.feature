...
Feature: Install local Helmfile observability with the control profile
  ...
  Scenario: Control profile installs shared infrastructure and control monitors
    ...
    When I run command "kubectl get opentelemetrycollector nvcf-observability -n monitoring --context k3d-ncp-local -o yaml"
    Then the command exit code should be 0
    And the command output should contain "targetAllocator:"
    And the command output should contain "enabled: true"
