...
// Package smoke owns target selection and phased execution for attach-mode
// smoke features. It reuses the shared strict DSL without adding target modes
// to the installation suite.
package smoke
...
func LoadTarget(path string) (Target, error) {
	...
	if err != nil {
		return Target{}, fmt.Errorf("read smoke target: %w", err)
	}
	decoder := yaml.NewDecoder(bytes.NewReader(raw))
	decoder.KnownFields(true)
	var target Target
	if err := decoder.Decode(&target); err != nil {
		return Target{}, fmt.Errorf("decode smoke target: %w", err)
	}
	var extra any
	if err := decoder.Decode(&extra); !errors.Is(err, io.EOF) {
		if err == nil {
			return Target{}, errors.New("decode smoke target: multiple YAML documents are not supported")
		}
		return Target{}, fmt.Errorf("decode smoke target: %w", err)
	}
	if target.Version != 1 {
		return Target{}, fmt.Errorf("smoke target version must be 1, got %d", target.Version)
	}
	name, err := dsl.InterpolateRequired(target.Name)
	if err != nil {
		return Target{}, fmt.Errorf("smoke target field name: %w", err)
	}
	target.Name = strings.TrimSpace(name)
	if target.Name == "" {
		return Target{}, errors.New("smoke target field name must not be empty")
	}
	if target.Env == nil {
	...
	for name, value := range target.Env {
		if err := validateEnvironmentName(name); err != nil {
			return Target{}, fmt.Errorf("smoke target env: %w", err)
		}
		if _, reserved := targetReservedEnvironment[name]; reserved {
			return Target{}, fmt.Errorf("smoke target env %s is runner-owned", name)
		}
		target.Env[name] = dsl.Interpolate(value)
	}
	if strings.TrimSpace(target.Env[cliConfigEnvironment]) == "" {
		return Target{}, fmt.Errorf("smoke target env %s must not be empty", cliConfigEnvironment)
	}
	return target, nil
}
